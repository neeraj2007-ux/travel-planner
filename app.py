...
# Setup Flask to serve frontend folder
...
def get_auth():
    token = request.headers.get('Authorization', '').split(' ')[1] if ' ' in request.headers.get('Authorization', '') else ''
...
def send_otp():
    ...
    # Fallback log for Render dashboard
    print(f"⚠️ EMAIL FAILED (Auth Service). OTP for {email} is {otp}")
    return jsonify({'success': True, 'message': 'OTP generated (Check Console)'})

@app.route('/api/verify-otp', methods=['POST'])
def verify_otp():
    data = request.json
    # Fix: Clean the email input here too
